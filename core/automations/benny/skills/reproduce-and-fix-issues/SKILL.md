...
Load the external Benny configuration supplied by the automation. If the config, a required action, the control adapter named by `control.skill_name`, or the completed feature map at `control.feature_map_path` is missing, malformed, or incomplete, stop at load, before any post or write, and record what was missing in the run output. Only a feature map with no section for the reported path, or an adapter capability that fails its check, ends the run `Blocked` at step 5. Neither ever ends in a guessed repro.

A run starts one of four ways, all set up by `setup-benny`: a native automation on Cursor; a CI or cron runner, with the trigger JSON on stdin; an issue assigned to the Copilot coding agent, with the trigger JSON in its body; or a manual run, where the operator pastes the trigger JSON, the source permalink, and the thread with each author's user ID, triage verdict included. The steps below are the same for all of them. A run with no Slack read access of its own is a manual handoff. It treats the paste as every thread read, posts nothing to Slack, and hands the operator each source or operations reply instead. A first manual run stops at step 9's repro report, without waiting out the rejection window, and also hands back the evidence paths and the media reviewer's answer from step 8. The fix phase is a second manual run whose paste carries that confirmed-repro reply as posted, the evidence paths, and the media answer. If the reply is older than the rejection window, no valid rejection follows it, and the evidence still exists at those paths, the run repeats steps 1 to 3 and 5, reuses the pasted evidence in place of steps 6 to 8, skips step 9, and continues at step 11. Otherwise it stops and says which condition failed.
...
Prefer the host's configured Slack actions: Cursor's native Slack actions, or a Slack MCP server on a runner. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
...
If any required capability is missing, mark the operations status as blocked and stop. An absent adapter already stopped the run at load. Do not pretend a screenshot, unit test, state mutation, or source reading is a UI repro.
