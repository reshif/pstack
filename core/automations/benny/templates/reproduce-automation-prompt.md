...
> Source material for the copied setup workflow. On Cursor, paraphrase this intent into a built-in `automate` draft after `automate` confirms that the copied pack is committed in the repository where the automation will run. On a CI or cron runner, fill it from configuration and pass it as the headless prompt, with the trigger JSON on stdin. On Copilot, put it and the trigger JSON in the assigned issue body.
