...
flowchart TD
    ...
    B --> C[Read the host binding; enter the mode]
    C --> D{Match the task and the repo's state}
    D -->|Already fixed or landed| L[Session pickup]
...
The [Worktree cleanup playbook](../../skills/poteto-mode/playbooks/worktree-cleanup.md) classifies every worktree by merge state, uncommitted work, and which chats still touch it. It deletes only what that evidence clears and pauses for your call on anything holding uncommitted edits or untracked files.
