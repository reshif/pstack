...
It writes `{{SKILL_DIR}}/control-ui/` for a browser, Electron, or desktop UI, or `{{SKILL_DIR}}/control-cli/` for a CLI or TUI, the names playbooks look for when they need a live proof. Inside are agent-facing instructions with exact Launch, Doctor, Drive, Evidence, and Cleanup sections, plus a feature map under `features/` that indexes what the app does and what result proves each feature works. The skill ships a [worked feature-map example](../../skills/create-verification-skill/references/feature-map-example/) with a README index and one file per feature using the four required H2s. Before handing it over, the generator proves the skill once end to end: launch, doctor check, drive one feature, capture evidence, clean up. If that proof still fails after three tries, it moves the draft out of the skill directory and reports what broke, so no playbook picks up an unproven skill. A checkout that doesn't start gets a report, not a skill.
