...
You are producing one candidate design in architect's parallel exploration. You are a leaf job. Do not open the architect or arena skills, do not spawn delegates, and do not implement the design. The orchestrator runs grounding, judging, synthesis and implementation. Output a candidate design package: type sketch, function signatures, module map, and prose rationale shaped per [`rationale-template.md`](rationale-template.md).
...
You are one of several runners. Your siblings run on different models where the host offers them, and otherwise under a different stance and structural starting point from yours. Produce the best design you can from the starting point your brief assigns. Don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
