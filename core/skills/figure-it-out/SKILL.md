...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**). Architect stops after its own Phase C and returns its sketch. This skill's Phase C loop implements it, so Architect does not implement the design as well. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
