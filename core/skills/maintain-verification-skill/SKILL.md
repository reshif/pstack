...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map, normally `{{SKILL_DIR}}/control-ui/` or `{{SKILL_DIR}}/control-cli/` (any `control-*/`, or a legacy `verify-*/`). None → outcome `blocked`: create nothing, and point at `/create-verification-skill` instead of inventing a target. Several → ask which one and wait; with nobody to ask, `blocked` with the candidate list rather than a guess. A legacy `verify-*` target is invisible to playbooks, which look up `control-ui` and `control-cli`, so renaming its directory and frontmatter `name` is an in-scope correction. If a skill already holds the new name, the outcome is `blocked`. References to the old name outside the skill's directory (a Benny `control.skill_name`, other skills, docs) are outside edit scope: list them in the report and leave them unedited.
