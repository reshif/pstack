...
This skill orchestrates three others: an inline mining pass (see step 1), your host's skill-authoring flow (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `{{SKILL_DIR}}/**/*-mode/SKILL.md` and `{{USER_SKILL_DIR}}/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`{{SKILL_DIR}}/<handle>/`), not only at the top level. If one exists, confirm intent with a structured question (`runtime/interaction.md`) (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Never glob across the host's other workspaces. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Ask a structured question (`runtime/interaction.md`) with multiple choice rather than asking the user to type from scratch.
...
Use your host's skill-authoring flow skill to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use `{{SKILL_DIR}}/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle. Otherwise default to `{{SKILL_DIR}}/<handle>-mode/SKILL.md` in the project (or `{{USER_SKILL_DIR}}/<handle>-mode/` if the user prefers a personal skill).
...
A `-mode` skill is subjective output. A benchmark-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.
