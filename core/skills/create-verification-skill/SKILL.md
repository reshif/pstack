...
Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as a project-local skill (`{{SKILL_DIR}}/control-<surface>/`) tailored to the repo. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
...
If the checkout doesn't build or start as-is, fix it only when the fix is small and outside product code (install dependencies, copy a documented `.env.example`, start a documented local service). Otherwise stop before generating and report the exact command, its failure, and what fixing it would take; a skill written against a broken base teaches wrong steps. When an irrelevant missing asset blocks startup (a static dir the API never serves, a sample config), the generated skill may create it, clearly marked as verification scaffolding, and remove it in cleanup.

## 2. Generate the skill

Name it by surface, because that is the name playbooks look up: `control-ui` for a browser, Electron, or desktop UI, and `control-cli` for a CLI or TUI. Playbooks ask for exactly those two "from the project's verification skills", so any other name for those surfaces is invisible to them. Another primary surface gets `control-<surface>` (`control-api`, `control-mobile`). Several apps on one surface share one skill, with a Launch section per app. If a skill by that name, or a legacy `verify-*` skill for the same surface, already exists, stop and point at `/maintain-verification-skill` instead of overwriting or duplicating it.

Write `{{SKILL_DIR}}/control-<surface>/SKILL.md` with YAML frontmatter (`name: control-<surface>`, matching the directory, and a `description` that names the app, the surface, and when to reach for it — without frontmatter the skill never registers) and these sections, each grounded in what the interview actually found (no placeholders left):
