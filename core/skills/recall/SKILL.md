...
Transcripts live in the host's session store (`runtime/host-profile.md` names it per host). Resolve the path for the active workspace only, and treat each line or entry as one chat message. If the host exposes no readable transcript, say so and rebuild context from the repo and the shared record instead.
