...
function frontierRoot(options: FrontierSetOptions): number | undefined {
  if (options.git === true && options.root === undefined) {
    throw new UsageError("--git needs --root <pr>");
  }
  if (options.git !== true && options.root !== undefined) {
    throw new UsageError("--root needs --git");
  }
  return options.root;
}
...
function createProgram(io: Io): Command {
  ...
  leaf(
    frontier,
    "set",
    "discover the stack (Graphite, or with --git its base-branch chain) and set the frontier"
  )
    .addOption(
      new Option(
        "--repo <dir>",
    ...
    .option(
      "--git",
      "walk the base-branch chain with gh instead of reading Graphite"
    )
    .option(
      "--root <pr>",
      "the stack's bottom pull request, required with --git",
      positiveInteger
    )
