...
// A forge with a merged root, a closed sibling next to the open child, an open
// top, and a separate root whose chain forks.
async function withFakeGh<T>({
  ...
  const fields = "--json number,headRefName,baseRefName,headRefOid,state";
  const row = (pr: number, head: string, base: string, sha: string, state: string) =>
    `{"number":${pr},"headRefName":"${head}","baseRefName":"${base}","headRefOid":"${sha}","state":"${state}"}`;
  await writeFile(
    gh,
    `#!/usr/bin/env bash
set -euo pipefail
case "$*" in
  "pr view 10 ${fields}")
    printf '%s\\n' '${row(10, "stack/merged", "main", SHA_A, "MERGED")}' ;;
  "pr list --base stack/merged --state all --limit 100 ${fields}")
    printf '%s\\n' '[${row(13, "stack/closed", "stack/merged", SHA_C, "CLOSED")},${row(11, "stack/open", "stack/merged", SHA_B, "OPEN")}]' ;;
  "pr list --base stack/open --state all --limit 100 ${fields}")
    printf '%s\\n' '[${row(12, "stack/top", "stack/open", SHA_C, "OPEN")}]' ;;
  "pr list --base stack/top --state all --limit 100 ${fields}")
    printf '[]\\n' ;;
  "pr view 20 ${fields}")
    printf '%s\\n' '${row(20, "stack/fork", "main", SHA_A, "OPEN")}' ;;
  "pr list --base stack/fork --state all --limit 100 ${fields}")
    printf '%s\\n' '[${row(21, "fork/a", "stack/fork", SHA_B, "OPEN")},${row(22, "fork/b", "stack/fork", SHA_C, "OPEN")}]' ;;
  *)
...
`;
  ...
  it("resolves a plain-git frontier from the base-branch chain", async () => {
    const { directory, store } = await initializedStore();
    const repo = join(directory, "repo");
    await mkdir(repo);
    ...
    await withFakeGh({
      directory,
      operation: async () => {
        expect(await store.frontier.set({ repo, root: 10 })).toEqual({
          generation: 1,
          prs: [
            { pr: 10, branches: "stack/merged", sha: SHA_A, state: "MERGED" },
            { pr: 11, branches: "stack/open", sha: SHA_B, state: "OPEN" },
            { pr: 12, branches: "stack/top", sha: SHA_C, state: "OPEN" },
          ],
          lowestUnmerged: 11,
        });
        expect(
          (await store.frontier.set({ repo, root: 10, prs: [10, 11, 12] }))
            .generation
        ).toBe(2);
        await expect(
          store.frontier.set({ repo, root: 10, prs: [10, 12, 11] })
        ).rejects.toThrow(
          "frontier pin mismatch: order differs: expected 10,12,11; git 10,11,12"
        );
        await expect(store.frontier.set({ repo, root: 20 })).rejects.toThrow(
          "the stack forks above stack/fork: PRs 21,22"
        );
      },
    });
  });

  it("points at --git when gt is not installed", async () => {
    const { directory, store } = await initializedStore();
    const repo = join(directory, "repo");
    const bin = join(directory, "empty-bin");
    await mkdir(repo);
    await mkdir(bin);
    const originalPath = process.env.PATH;
    ...
    try {
      await expect(store.frontier.set({ repo })).rejects.toThrow(
        "gt is not installed; pass --git --root <pr>"
      );
    } finally {
      process.env.PATH = originalPath;
    }
...
describe("orch CLI", () => {
  it("prints commander help and rejects invalid parsing with exit 1", async () => {
    ...
    expect(frontierHelp.stdout).toContain("--git");
    expect(frontierHelp.stdout).toContain("--root <pr>");
  ...
  it("maps user and not-found outcomes to the preserved exit codes", async () => {
    ...
    const gitWithoutRoot = runCli([
      "--store", directory, "frontier", "set", "--repo", directory, "--git",
    ]);
    expect(gitWithoutRoot.code).toBe(1);
    expect(gitWithoutRoot.stderr).toContain("--git needs --root <pr>");
    const rootWithoutGit = runCli([
      "--store", directory, "frontier", "set", "--repo", directory, "--root", "10",
    ]);
    expect(rootWithoutGit.code).toBe(1);
    expect(rootWithoutGit.stderr).toContain("--root needs --git");
    ...
    await withFakeGh({
      ...
      operation: async () => {
        const plain = runCli([
          "--store", directory, "frontier", "set", "--repo", repo, "--git", "--root", "10",
        ]);
        expect(plain.stderr).toBe("");
        expect(plain.code).toBe(0);
        expect(plain.stdout).toBe(
          `generation=1 prs=stack/merged#10@${SHA_A}:MERGED,stack/open#11@${SHA_B}:OPEN,stack/top#12@${SHA_C}:OPEN lowest-unmerged=11\n`
        );
      },
    });
