...
export interface SetFrontierParams {
  ...
  // The stack's bottom PR. When set, the frontier comes from the base-branch
  // chain via gh instead of from Graphite.
  ...
  try {
    ...
    throw new UserError(
      errorCode(error) === "ENOENT"
        ? "gt is not installed; pass --git --root <pr> to walk the base-branch chain instead"
...
const GH_PR_FIELDS = "number,headRefName,baseRefName,headRefOid,state";
...
function ghJson({
  ...
  try {
    ...
    throw new UserError(`gh ${args.join(" ")} did not print JSON`);
  }
}

function ghFrontierRow(value: unknown, context: string): FrontierPr & {
  ...
  if (
    ...
    throw new UserError(`gh output has an invalid PR row for ${context}`);
  }
  return {
...
// Plain-git stacks: walk the base-branch chain up from the root PR. Each PR's
// children are the PRs based on its head branch. A fork is an error, not a guess.
function gitFrontier({
  repo,
  root,
}: {
  repo: string;
  ...
  const chain = [
    ghFrontierRow(
      ghJson({
        repo,
        args: ["pr", "view", String(root), "--json", GH_PR_FIELDS],
      }),
      `PR #${root}`
    ),
  ];
  for (let top = chain[0]; top !== undefined; top = chain[chain.length - 1]) {
    const listed = ghJson({
      repo,
      args: [
        "pr",
    ...
    if (!isUnknownArray(listed)) {
      throw new UserError(`gh pr list --base ${top.branches} did not print a list`);
    }
    const children = listed.map((row) =>
      ghFrontierRow(row, `base ${top?.branches ?? ""}`)
    );
    const live = children.filter((row) => row.state !== "CLOSED");
    const next = live.length > 0 ? live : children;
    if (next.length === 0) {
      break;
    }
    if (next.length > 1) {
      throw new UserError(
        `the stack forks above ${top.branches}: PRs ${next.map((row) => row.pr).join(",")}; pass the root of one chain`
      );
    }
    const child = next[0];
    if (
      child === undefined ||
      chain.some((row) => row.pr === child.pr || row.branches === child.branches)
    ) {
      throw new UserError(`base-branch chain loops at ${child?.branches ?? top.branches}`);
    }
    chain.push(child);
  }
  return chain.map(({ branches, pr, state, sha }) => ({ branches, pr, state, sha }));
}
      ...
      set: async (params) => {
        ...
        const root =
          params.root === undefined
            ? undefined
            : positiveInteger(params.root, "--root");
        const old = await readFrontier(store);
        const prs =
          root === undefined
            ? resolveFrontier(repo)
            : gitFrontier({ repo, root });
        if (pin !== undefined) {
          validateFrontierPin({
            actual: prs.map((row) => row.pr),
            expected: pin,
            source: root === undefined ? "gt" : "git",
          });
        }
