# the automated reviewer triage

Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles the automated reviewer or review-automation comments. The goal is not to ignore the automated reviewer by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each the automated reviewer thread before acting:
...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the automated reviewer's comment is only restating that a shared visual default changed.
...
- Confidence: candidate
- Skip when: the automated reviewer flags an export, component, helper, or file as unused, and the active forge's PR list and diffs, upper-stack diffs, or PR context show it is used by a later PR in the stack.
...
- Skip when: The comment body or a later the automated reviewer reply explicitly says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Source: one sticky-occlusion PR: six the automated reviewer passes, roughly eighteen findings, every one fixed rather than dismissed.
...
- Source: one prose-pinning PR with eight the automated reviewer passes; the claim was real on
