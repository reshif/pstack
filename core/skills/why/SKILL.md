...
Before spawning investigators, list the MCP servers reachable in this session. Use the host's available-tools map when it exposes one. Otherwise read the host's MCP configuration (`.mcp.json`, `~/.claude.json`, `~/.codex/config.toml`, or the host's settings) for enabled servers.
...
- `model`: your configured why-investigators model (default the `fast` class)
...
- `role`: `pstack-reviewer` (read-only delegate)
- `model`: your configured why-synthesizer model (default the `deep` class)
- `access`: `read`. The delegate may call tools, including MCP tools. It must not write.
