...
Invoked at the end of every playbook that ships a code change, and for each PR that Autonomous run, Orchestrate, and the Autopilots open. Investigation, Runtime forensics, Trace forensics, Prototype, Eval, Multi-phase plan, Pause safely, Session pickup, and Worktree cleanup end without one, and Babysit and Shipping work on PRs that already exist.

**Worktree.** Work from a git worktree off main. Start the run record inside that worktree, or move it there before any evidence with `rr workspace --path <worktree>`, run from the worktree. Subagents inherit it. A delegate in its own worktree records nothing in the run; the parent records its return and re-verifies in the run's workspace. Multiple delegate calls on the same branch each get their own worktree, or `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.

**Commits.** Commit liberally. Rebase into small, ordered commits before opening PRs. Each commit is a future PR: landable, ordered to tell the story. Amend when the fix belongs in a just-made commit. New commit when separable.

**PRs.** Strip code slop from the diff before commit: unnecessary comments, defensive try/catch in trusted paths, `any` casts that bypass typing, and needless nesting. Upstream does this with `/deslop` from `cursor-team-kit`, which this port does not bundle. Run that skill if you have it, otherwise do the same pass in plain words. This is a separate gate from `/unslop`, which is prose only. Run `/no-comments` before review. Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
...
**Stale proof.** The slop pass and `/no-comments` can change code after the calling playbook verified it. A code or harness change here makes that verification stale. Rerun the calling playbook's verify step on the final tree before you create the PR. When the calling playbook keeps a run record, run its `check` through the step before this one, and do not create the PR while it reports a problem.

A subagent that opens a PR runs the code-slop pass above and `/no-comments`, using its Without delegation branch. It does not run `interrogate`, because interrogate spawns a panel and a subagent cannot. When the calling playbook requires interrogate, the parent runs it on the final diff before delegating PR creation. The subagent returns the URL and does not babysit. Return to the parent.
