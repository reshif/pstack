...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the delegation protocol (`runtime/delegation.md`). State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Track coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. A track coordinator is a peer standing session, started the way this chat was, never a spawned delegate: a delegate is a leaf job that does not spawn, and most hosts allow one level of nesting at most (`runtime/delegation.md`, Reaching a routed skill). On a host that cannot start a top-level session itself, the human opens it: park that request in `gates.md` with the track's stored brief. Each extra session re-pays a full orientation preamble. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers as leaf jobs from its own session. Rolls up aggregates at wave boundaries into the program store through `orch inbox push --report`. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always `location: remote`, `isolation: dir` unless the task needs this machine: `control-ui` or `control-cli` runtime verification (from the project's verification skills). Reading local transcripts under `agent-transcripts/`. Simulators and local IDE state. Auth that exists only here. Cloud agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.

Delegation depth stays at one: a coordinator, or a track coordinator, spawns workers, and workers spawn nothing. Author the track decomposition per project (build, landing, and verification are common cuts, not a required shape). Hard-coded swarm trees were tried and parked as too rigid.
...
A track coordinator's standing brief adds its track boundary and unit list, its spawn budget with the cloud default and the local exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).

A dependency is a context relay, not just ordering. Undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Audit one sampled worker brief per track coordinator per wave, concurrently with the wave it samples, never as a gate in front of it. A failing brief stops that track and fixes the track coordinator's instructions, not just the worker, because brief quality decays late in a run. Never resume-chain a brief. Respawn fresh with consolidated scope.

#### Steps

1. **Frame.** State the done predicate as something countable ("all 126 units merged, each ledger-verified `unit-test-verified` or better"). Quantify scope: units, rough effort, expected stacks, and the wall-clock budget. If one agent could finish inside that budget, stop here and run Autonomous run instead. Record step 1 done, then `rr reroute --route autonomous-run --reason "<why one agent suffices>"`, where `rr` is the run-record command in `references/session-tracking.md`. That closes this run record into a new Autonomous run record, and `rr check` then reports the new one. Collapsing must not depend on another document being present. It means do the work directly in this session, plain workers where they help, verification inline, landing as you go, and none of the store, register, or pilot machinery below. Schedule landing against the budget. By roughly 70% of it, stop spawning and land what is verified. Name the tracks per project. A contested decomposition or one-way door goes through the arena skill before the pilot. Present the framing once. Reversible prep proceeds without waiting.
2. **Install the runtime.** Run `orch init`. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs per Stack safety.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` after every merge and stack mutation: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Graphite (`gt`) is optional. A stack is a base-branch chain per Opening a PR, **Size and stacks**. Without gt, `orch frontier set --repo <repo-dir> --git --root <root-pr>` walks that chain up from the root PR with `gh` (`gh pr view`, then `gh pr list --base <head>` for each PR's child), takes each head SHA from the forge, and writes `frontier.json`, bumping the generation. It errors on a fork rather than guessing. On a forge without `gh`, the stacker walks the chain with the resolved forge (`origin pr view <pr>`), reads each head SHA with `git rev-parse` after `git fetch`, and writes `frontier.json` by hand in the shape `orch frontier show` prints (`generation`, `prs` rows of `pr`, `branches`, `sha`, and `state` OPEN, MERGED or CLOSED, `lowestUnmerged`). Where the project tracks the stack in gt, `orch frontier set --repo <repo-dir>` without `--git` computes it instead, because GitHub base refs drift mid-restack while gt tracking is authoritative. Resolve it where gt knows the stack, normally the stacker's clone. A checkout whose gt metadata never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may change its topology (restack, retarget, and `gt` where the project uses it), serialized within its stack. Record the holder in the standing orders. Restacks run in cloud. A local restack at this scale takes the laptop down.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the delegate's status as the host reports it. Transcript mtime is not liveness.
...
- Retry by mode, once: cap-hit or oom, respawn with smaller scope. Network-drop, retry as-is. Tool-error, retry on a different model. Unknown, retry as-is with the failure named. A unit gets two attempts, the first and one retry, per `runtime/delegation.md`, **One retry**. A second failure abandons the unit. Record it dropped and replan around it.
