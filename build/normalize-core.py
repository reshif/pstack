...
# The one list lives in build/upstream.json, next to the upstream hash each rewrite
# was derived from, so the recipe, check-upstream.py and all four passes agree.
PORT_OWNED = list(json.loads((ROOT / "build" / "upstream.json").read_text())["port_owned"])
