...
build/port/upstream.json and restores them over the top of any re-derivation. That protects the rewrites and silently discards upstream's own
...
what upstream ships now.
    ...
    python3 build/port/check-upstream.py /path/to/cursor-plugins [--rev REV]
"""
import hashlib
import json
import pathlib
...
UP = json.loads((ROOT / "build" / "port" / "upstream.json").read_text())
...
def main() -> int:
    ...
    if conflicts:
        ...
        print("\nMerge upstream's change into core/, then update build/port/upstream.json.")
        return 1
